...
def process_file(input_path, output_path):
    ...
    # Ensure Part of Speech and Grammar exist in header: insert after UID (keeping original columns)
    # If they already exist, keep their position.
    if "Part of Speech" not in header:
        insert_pos = uid_idx + 1
        header.insert(insert_pos, "Part of Speech")
    if "Grammar" not in header:
        # if we inserted Part of Speech, Grammar should come right after it
        pos_idx = header.index("Part of Speech")
        header.insert(pos_idx + 1, "Grammar")
    ...
    for row in data_rows:
        # ensure row length matches original header before insertions; extend to current header length
        if len(row) < len(header):
            row = row + [""] * (len(header) - len(row))
        else:
            # if row longer than header (rare), still keep values
            row = row[:len(header)]
        ...
        # only fill POS/Grammar if empty
        if token:
            if not row[pos_idx].strip():
                row[pos_idx] = readable_pos(token.pos_)
            if not row[gram_idx].strip():
