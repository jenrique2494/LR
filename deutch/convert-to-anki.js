...
const structuredWords = JSON.parse(fs.readFileSync('deutch/structured-german-words_1.json', 'utf8'));
...
Object.values(structuredWords).forEach(wordData => {
  ...
  const slug = wordData.slug;
  const rank = wordData.rank;
  ...
  const tags = `${wordData.frequency} ${getLevel(wordData.frequency)}`;
...
fs.writeFileSync('anki_cards_german_1.csv', csvContent, 'utf8');

console.log(`Successfully converted ${csvRows.length} words to anki_cards_german_1.csv`);
